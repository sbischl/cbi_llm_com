...
dom[named_dom %in% dominance_to_plot] %>%
  map(function(dominance) {
    two_stage_ls <- feols(
      as.formula(str_glue("{dominance} ~ l1_{dominance} + diff_inflation + diff_unemployment  | country + year | ro_cbie_index ~ l1_inverse_distance_weighted_cbi_world + l1_electoral_democracy_10 + independence_judiciary")),
      data = speech_agg_instruments,
      cluster = ~country
    )
      return(summary(two_stage_ls, stage = 1))
  }) %>%
  append(
    dom[named_dom %in% dominance_to_plot] %>%
  map(function(dominance) {
  ...
  etable(depvar = T,
         ...
         dict = c("ro_cbie_index" = "$\\widehat{\\textrm{CBI}}$",
                  ...
                  "country" = "Country",
                  "year" = "Year",
                  "l1_monetary_dominance" = "Monetary dominance 25 prior speeches",
                  "l1_financial_dominance" = "Financial dominance 25 prior speeches",
         ...
         order = c("\\$\\\\widehat\\{\\\\textrm\\{CBI\\}\\}\\$",
                   "Monetary dominance 25 prior speeches", "Financial dominance 25 prior speeches",
                   "\\$\\\\Delta\\$"),
         tex = TRUE,
         fitstat = ~r2 + n,
         style.tex = style.tex("base",
                               ...
                               yesNo = "$\\checkmark$"),
         headers = list("^:_:" = list("First Stage" = 2, "2SLS Effect on Dominance" = length(dominance_to_plot)))) %>%
  write_lines("./output/tables/iv/iv_country_fe_reg.tex")